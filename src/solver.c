...
/* CODE ADDED */
void reverseOutput(char **strings, char **newStrings, int f_length)
{
   for (int i = 0; i < f_length; i++)
   {
      newStrings[i] = malloc(strlen(strings[f_length - i - 1]) * sizeof(char));
      newStrings[i] = strings[f_length - i - 1];
   }
}

void oppositeOutput(char **strings, int f_length)
{
   for (int i = 0; i < f_length; i++)
   {
...
void rewriteOutput(char **strings, int f_length)
{
   char **newStrings;
   newStrings = malloc(f_length * sizeof(char *));

   reverseOutput(strings, newStrings, f_length);

   oppositeOutput(newStrings, f_length);

   writeToFile(newStrings, f_length);
...

{
   ...
   /* CODE ADDED */
   strings = malloc(f_length * sizeof(char *));
   /* CODE ADDED */
   ...
   for (ii = 0; ii < count; ii++)
   {
      ...
      /* CODE ADDED */
      strings[ii] = malloc(strlen(twist_string[turn_list[ii]]) * sizeof(char));
      strcpy(strings[ii], twist_string[turn_list[ii]]);
      /* CODE ADDED */
   }
   ...
   /* CODE ADDED */
   rewriteOutput(strings, f_length);
   free(strings);

   exit(EXIT_SUCCESS); // Exit when 1 solution was found
   /* CODE ADDED */
